...
def get_args():
    ...
    parser.add_argument('-tk', '--token', action='append',
                        help='Token for account.')
    ...
    if args.only_server:
        ...
        for i, username in enumerate(args.username):
            args.accounts.append({'username': username, 'password': args.password[i], 'auth_service': args.auth_service[i], 'token': None})
        for i, token in enumerate(args.token):
            args.accounts.append({'username': '', 'password': '', 'token': token, 'auth_service': args.auth_service[i]})
